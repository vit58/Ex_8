...
# Create your models (МОДЕЛЬ)here.
# Объявляем Djang-е что это класс, который нужно отобразить в БД
class ProductCategory(models.Model):
    name = models.CharField(max_length=64, unique=True, verbose_name='название')
    description = models.TextField(verbose_name='описание')
    is_active = models.BooleanField(default=True)

    # Рекомендуеся заполнять для такой модели метод str у класса
    def __str__(self):
        return f'{self.name}'
    ...
    class Meta:
        ...
        ordering = ('-id',)
        # Кортеж с одним параметром. Нужна ОБЯЗАТЕЛЬНО запятая!!!
